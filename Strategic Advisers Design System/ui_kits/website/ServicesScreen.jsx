      ...
      <section style={{ padding: 'var(--section-y-tight) 0' }}>
        <div style={{ maxWidth: 'var(--container-max)', margin: '0 auto', padding: '0 var(--gutter)', display: 'flex', flexDirection: 'column', gap: '20px' }}>
          {window.SA_SERVICES.map((s, i) => (
            <div key={s.title} style={{ display: 'grid', gridTemplateColumns: '64px 1fr auto', gap: '28px', alignItems: 'center', background: '#fff', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-lg)', padding: '28px 32px', boxShadow: 'var(--shadow-xs)' }} className="sa-service-row">
              <span style={{ width: '64px', height: '64px', borderRadius: 'var(--radius-md)', background: 'var(--navy-50)', color: 'var(--navy-700)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}><Icon name={s.icon} size={30} color="var(--navy-700)" /></span>
              <div>
      ...
      <section style={{ background: 'var(--surface-subtle)', padding: 'var(--section-y) 0' }}>
        <div style={{ maxWidth: 'var(--container-max)', margin: '0 auto', padding: '0 var(--gutter)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '20px', marginTop: '48px' }} className="sa-grid-4">
            {process.map((p) => (
              <div key={p.n} style={{ background: '#fff', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-lg)', padding: '28px 26px' }}>
