...
export function IndustryCard({ label, count, image, href, style, ...rest }) {
  ...
  return (
    ...
    >
      ...
      <span style={{ position: 'absolute', top: 0, left: 0, height: '5px', width: hover ? '100%' : '0%', background: 'var(--red-600)', transition: 'width var(--dur-base) var(--ease-out)' }} />
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, padding: '24px', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: '12px' }}>
        ...
        <span style={{ flexShrink: 0, width: '40px', height: '40px', border: '2px solid rgba(255,255,255,0.5)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', color: '#fff', background: hover ? 'var(--red-700)' : 'transparent', borderColor: hover ? 'var(--red-700)' : 'rgba(255,255,255,0.5)', transition: 'all var(--dur-base) var(--ease-out)' }}>→</span>
      </div>
    </a>
  );
}
