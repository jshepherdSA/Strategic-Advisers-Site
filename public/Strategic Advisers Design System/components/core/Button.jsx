...
export function Button({
  ...
  const variants = {
    primary: { background: 'var(--red-700)', color: '#fff', border: '1px solid var(--red-700)', boxShadow: 'none' },
    secondary: { background: 'var(--navy-700)', color: '#fff', border: '1px solid var(--navy-700)', boxShadow: 'none' },
    outline: { background: 'transparent', color: 'var(--navy-800)', border: '1px solid var(--navy-400)', boxShadow: 'none' },
    ghost: { background: 'transparent', color: 'var(--navy-700)', border: '1px solid transparent', boxShadow: 'none' },
    'on-navy': { background: '#fff', color: 'var(--navy-900)', border: '1px solid #fff', boxShadow: 'none' },
  ...
  const hoverMap = {
    primary: (e, on) => { e.currentTarget.style.background = on ? 'var(--red-800)' : 'var(--red-700)'; e.currentTarget.style.borderColor = on ? 'var(--red-800)' : 'var(--red-700)'; },
    secondary: (e, on) => { e.currentTarget.style.background = on ? 'var(--navy-900)' : 'var(--navy-700)'; e.currentTarget.style.borderColor = on ? 'var(--navy-900)' : 'var(--navy-700)'; },
    outline: (e, on) => { e.currentTarget.style.background = on ? 'var(--navy-900)' : 'transparent'; e.currentTarget.style.color = on ? '#fff' : 'var(--navy-800)'; },
  ...
  const handlers = disabled ? {} : {
    ...
    onMouseLeave: (e) => { hoverMap[variant]?.(e, false); e.currentTarget.style.transform = 'translate(0,0)'; },
