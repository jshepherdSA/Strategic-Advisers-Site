  ...
  return (
    <section style={{ background: 'var(--surface-subtle)', minHeight: '78vh' }}>
      <div style={{ maxWidth: 'var(--container-max)', margin: '0 auto', padding: 'clamp(3rem,6vw,5.5rem) var(--gutter)', display: 'grid', gridTemplateColumns: '0.95fr 1.05fr', gap: '56px', alignItems: 'start' }} className="sa-hero-grid">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', marginTop: '40px' }}>
            {contacts.map((c) => (
              <div key={c.h} style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                <span style={{ width: '46px', height: '46px', background: 'var(--navy-900)', border: '2px solid var(--navy-900)', color: '#fff', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><Icon name={c.icon} size={20} color="#fff" /></span>
                <div>
        ...
        <div style={{ background: '#fff', borderRadius: 'var(--radius-xl)', boxShadow: 'var(--shadow-md)', padding: 'clamp(1.75rem,3vw,2.75rem)' }}>
          {sent ? (
            <div style={{ textAlign: 'center', padding: '32px 8px' }}>
              <span style={{ width: '64px', height: '64px', background: 'var(--green-100)', color: 'var(--color-success)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', marginBottom: '20px' }}><Icon name="check" size={32} color="var(--color-success)" /></span>
