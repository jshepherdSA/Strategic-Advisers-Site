...
- **Spacing & layout:** 4px base unit, generous section rhythm (`--section-y` ≈ 4–8rem). Centered containers (max 1200/1360px) with fluid gutters. Whitespace is deliberate and premium — pages breathe. **Layout language is Mondrian / De Stijl:** asymmetric rectangular blocks arranged on a grid, but separated by **clean whitespace gaps** (no ruled lines) rather than borders. Composition blocks are flat fields of navy, red or white — see the hero and the "Composition Grid" brand card.
- **Backgrounds:** predominantly white and flat light navy-tinted `--navy-50` — **no gradients.** Dark **solid navy blocks** (`--navy-800`) mark high-emphasis moments: stat bands, CTA sections, footer. Real **photography** appears inside rectangular grid cells, always under a flat navy scrim so text stays legible. No hand-drawn illustration, no busy patterns, no soft/tech gradients — flat color fields only.
- **Imagery treatment:** cool, slightly desaturated (`saturate(0.9)`), professional and grounded — no heavy filters, no grain. Placed as flat rectangular cells in the composition grid.
- **Cards:** white surface, **no border**, **squared corners (0 radius)**, a soft low shadow (`--shadow-sm`) at rest. On hover a card lifts by translating up-left `translate(-5px,-5px)` and casts a **light hard-offset block shadow** (`--shadow-block-red` / `--shadow-block`) — the graphic "edge" of the system, done with color-tinted shadow rather than outlines. Service cards reveal a red top rule and flip their square icon tile from navy to red. Industry cards are image-forward (3:4), navy scrim, square arrow badge.
- **Elevation:** subtle blurred shadows at rest (`--shadow-xs → --shadow-xl`, cool navy-tinted); on hover, a **light hard-offset block shadow** (`--shadow-block`, `--shadow-block-red` at 12px, ~8–16% alpha) gives the flat/graphic edge. Nothing uses heavy or black outlines.
- **Corner radius:** **squared — all radius tokens are 0.** Right angles are a brand rule (Mondrian). The only curves are within photographic content itself.
- **Borders:** **avoided.** Separation comes from background fills, whitespace gaps and light shadows — not outlines. Where a hairline is unavoidable (form inputs, small chips) it is a single thin `--border-subtle`/`--border-default` line, never heavy or black. Buttons use a 1px same-color (or light navy for outline) edge only.
- **Motion:** crisp and mechanical, no bounce. Standard ease `cubic-bezier(.2,0,0,1)`, softer `--ease-out` for reveals. Durations 120/200/340ms. Transitions: color on links, block-shadow + translate lift on cards, red rule sweep, arrow nudge (`translateX(4px)`), image zoom. Buttons press with a 1px down-right nudge (`translate(1px,1px)`). No infinite/decorative loops.
- **Hover states:** buttons invert/darken and keep a flat fill (outline fills solid navy with white text); links shift navy→red; cards lift up-left + cast a light block shadow + reveal a red rule; social icons fill red.
- **Press states:** buttons nudge `translate(1px,1px)` — a small, tactile, mechanical confirm.
